...
var KTDatatablesExtensionsKeytable = function () {
    ...
    var initTable = function () {
        // begin first table
        var table = $('#deletedCategoriesTable').DataTable({
            language: {
                "sDecimal": ",",
        ...
        $('#btnRefresh').click(function () {
            ...
            $.ajax({
                type: 'GET',
                url: '/Admin/Kategori/TumSilinmisKategoriler/',
                ...
                success: function (data) {
                    ...
                    if (categoryListDto.ResultStatus === 0) {
                        $.each(categoryListDto.Categories.$values,
                            function (index, category) {
                                const newTableRow = table.row.add([
                                    ...
                                    `
	                        <button id="btnUndo" class="btn btn-sm btn-clean btn-icon mr-2" title="Geri Al" data-id="${category.Id}">
                            ...
                            <button id="btnDelete" class="btn btn-sm btn-clean btn-icon" title="Kalıcı Olarak Sil" data-id="${category.Id}">
                                <span class="svg-icon svg-icon-md">
                        ...
                        toastr.error(`${categoryListDto.Data.Message}`, 'İşlem Başarısız!');
                    }
                },
                error: function (err) {
                    $('.spinner-border').hide();
        ...
        $(document).on('click',
            ...
            function (event) {
                ...
                Swal.fire({
                    ...
                    text: `${categoryName} başlıklı kategori kalıcı olarak silinicektir!`,
                    icon: 'warning',
            ...
            if (seciliDosyalar.length > 0 && topluSecim === true) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: 'POST',
                            url: '/Admin/Kategori/TopluKaliciSil',
        ...
        $(document).on('click',
            ...
            function (event) {
                ...
                Swal.fire({
                    ...
                    text: `${categoryName} başlıklı kategori çöp kutusundan geri getirilecektir!`,
                    icon: 'question',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
            ...
            if (seciliDosyalar.length > 0 && topluSecim === true) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: 'POST',
                            url: '/Admin/Kategori/TopluGeriAl',
                            data: { categoryIds: ids },
                            success: function (data) {
                                const categoryDto = jQuery.parseJSON(data);
                                if (categoryDto.ResultStatus === 0) {
                                    Swal.fire({
                                        title: 'Silindi!',
                                        text: `Seçilen ${seciliDosyalar.length} kayıt çöp kutusundan geri getirildi.`,
                                        type: 'success'
                                    }).then(function () {
                                        seciliDosyalar.each(function () {
                                            var deletedRow = $(this).closest("tr");
