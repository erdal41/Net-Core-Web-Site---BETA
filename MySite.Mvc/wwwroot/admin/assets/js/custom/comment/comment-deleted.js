...
var KTDatatablesExtensionsKeytable = function () {
    ...
    var initTable = function () {
        // begin first table
        var table = $('#deletedCommentsTable').DataTable({
            language: {
                "sDecimal": ",",
        ...
        $('#btnRefresh').click(function () {
            ...
            $.ajax({
                type: 'GET',
                url: '/Admin/Yorum/TumSilinmisYorumlar/',
                ...
                success: function (data) {
                    ...
                    if (commentResult.Data) {
                        ...
                        $.each(commentResult.Data.Comments.$values,
                            function (index, comment) {
                                ...
                                const newTableRow = table.row.add([
                                    ...
                                    `${newComment.IsActive ? "Onaylandı" : "Onay Bekliyor"}`,
                                    `${convertToShortDate(newComment.ModifiedDate)}`,
                                    newComment.ModifiedByName,
                                    `
	                        <button id="btnUndo" class="btn btn-sm btn-clean btn-icon mr-2" title="Geri Al" data-id="${comment.Id}">
                            ...
                            <button id="btnDelete" class="btn btn-sm btn-clean btn-icon" title="Kalıcı Olarak Sil" data-id="${comment.Id}">
                                <span class="svg-icon svg-icon-md">
                        ...
                        toastr.error(`${commentListDto.Data.Message}`, 'İşlem Başarısız!');
                    }
                },
                error: function (err) {
                    $('.spinner-border').hide();
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            dataType: 'json',
                            data: { commentId: id },
                            url: '/Admin/Yorum/KaliciSil/',
                            success: function (data) {
                                const commentResult = jQuery.parseJSON(data);
                                if (commentResult.ResultStatus === 0) {
                                    Swal.fire(
                                        'Silindi!',
                                        `${commentText} içerikli yorum başarıyla silinmiştir.`,
                                        'success'
                                    ).then(function () {
        ...
        $('#btnMultiDelete').click(function () {
            ...
            if (seciliDosyalar.length > 0 && topluSecim === true) {
                ...
                Swal.fire({
                    ...
                    text: `Seçilen ${seciliDosyalar.length} kaydı kalıcı olarak silmek istediğinizden emin misiniz?`,
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: 'POST',
                            url: '/Admin/Yorum/TopluKaliciSil',
                            data: { commentIds: ids },
                            success: function (data) {
                                const commentResult = jQuery.parseJSON(data);
                                if (commentResult.ResultStatus === 0) {
                                    Swal.fire(
                                        ...
                                        `Seçilen ${seciliDosyalar.length} kayıt kalıcı olarak silindi.`,
                                        'success'
                                    ).then(function () {
                                        seciliDosyalar.each(function () {
        ...
        $(document).on('click',
            ...
            function (event) {
                ...
                const id = $(this).attr('data-id');
                const tableRow = $(`[name="${id}"]`);
                const commentText = tableRow.find('td:eq(2)').text();
                Swal.fire({
                    title: 'Çöp kutusundan geri getirmek istediğinize emin misiniz?',
                    text: `${commentText} içerikli yorum çöp kutusundan geri getirilecektir!`,
                    icon: 'question',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
        ...
        $('#btnMultiUndo').click(function () {
            ...
            if (seciliDosyalar.length > 0 && topluSecim === true) {
                ...
                Swal.fire({
                    title: 'Geri Alma',
                    text: `Seçilen ${seciliDosyalar.length} kayıtları çöp kutusundan geri almak istediğinizden emin misiniz?`,
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                ...
                                if (commentResult.Data) {
                                    Swal.fire(
                                        'Silindi!',
                                        `Seçilen ${seciliDosyalar.length} kayıt çöp kutusundan geri getirildi.`,
                                        'success'
                                    ).then(function () {
                                        seciliDosyalar.each(function () {
                                            var deletedRow = $(this).closest("tr");
