...
var KTDatatablesExtensionsKeytable = function () {
    ...
    var initTable = function () {
        // begin first table
        var table = $('#deletedTagsTable').DataTable({
            language: {
                "sDecimal": ",",
        ...
        $('#btnRefresh').click(function () {
            ...
            $.ajax({
                type: 'GET',
                url: '/Admin/Etiket/TumSilinmisEtiketler/',
                ...
                success: function (data) {
                    ...
                    if (tagListDto.ResultStatus === 0) {
                        $.each(tagListDto.Tags.$values,
                            function (index, tag) {
                                const newTableRow = table.row.add([
                                    tag.Id,
                                    tag.Name,
                                    tag.IsActive ? "Yayında" : "Taslak",
                                    convertToShortDate(tag.ModifiedDate),
                                    tag.ModifiedByName,
                                    `
	                        <button id="btnUndo" class="btn btn-sm btn-clean btn-icon mr-2" title="Geri Al" data-id="${tag.Id}">
                            ...
                            <button id="btnDelete" class="btn btn-sm btn-clean btn-icon" title="Kalıcı Olarak Sil" data-id="${tag.Id}">
                                <span class="svg-icon svg-icon-md">
                        ...
                        toastr.error(`${tagListDto.Data.Message}`, 'İşlem Başarısız!');
                    }
                },
                error: function (err) {
                    $('.spinner-border').hide();
        ...
        $(document).on('click',
            ...
            function (event) {
                ...
                Swal.fire({
                    ...
                    text: `${tagName} başlıklı etiket kalıcı olarak silinicektir!`,
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            dataType: 'json',
                            data: { tagId: id },
                            url: '/Admin/Etiket/KaliciSil/',
                            success: function (data) {
                                const tagDto = jQuery.parseJSON(data);
                                if (tagDto.ResultStatus === 0) {
                                    Swal.fire(
                                        'Silindi!',
                                        `${tagDto.Message}`,
                                        'success'
            ...
            if (seciliDosyalar.length > 0 && topluSecim === true) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            type: 'POST',
                            url: '/Admin/Etiket/TopluKaliciSil',
                            data: { tagIds: ids },
                            success: function (data) {
                                const tagDto = jQuery.parseJSON(data);
                                if (tagDto.ResultStatus === 0) {
        ...
        $(document).on('click',
            ...
            function (event) {
                ...
                Swal.fire({
                    ...
                    text: `${tagName} başlıklı etiket çöp kutusundan geri getirilecektir!`,
                    icon: 'question',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
