...
namespace MySite.Shared.Utilities.Extensions
{
        ...
        public static async Task<string> RenderViewToStringAsync(this Controller controller, string viewNamePath)
        {
            ...
            using (StringWriter writer = new StringWriter())
            {
                try
                {
                    ...
                    if (viewNamePath.EndsWith(".cshtml"))
                    {
                        if (viewEngine != null) viewResult = viewEngine.GetView(viewNamePath, viewNamePath, false);
                    }
                    ...
                    if (viewResult == null) return writer.GetStringBuilder().ToString();
